...
public class FunctionsLoadfilesDetails {
    public static void leituraDetalhes(String ficheiroDetails) {
        try {
            Main.listadetalhes = new HashSet<Song>();
            Main.listaDetalhes2 = new ArrayList<Song>();
            Main.infoSongs1 = new ParseInfo(0,0);
            ...
            while (leitorFicheiro.hasNextLine()) { // enquanto tiver linhas...
                ...
                if (dados.length == 7) {
                    ...
                    if (ID.equals("")) { // id inexistente
                        Main.infoSongs1.numLinhasIgnored++;
                    } else {
                        if (!guardaIDs2.add(ID)) {
                            Main.infoSongs1.numLinhasIgnored++;
                            EncontrouDuplicadoId2 = 1; // uma musica ja tem esse id
                        }
                        if (EncontrouDuplicadoId2 == 0) { //caso seja uma musica valida
                            ...
                            Main.listadetalhes.add(detalhes);
                            Main.listaDetalhes2.add(detalhes);
                            guardaIDs2.add(ID);
                            Main.infoSongs1.numLinhasOk++;
                        }
                    }
                } else {
                    Main.infoSongs1.numLinhasIgnored++;
